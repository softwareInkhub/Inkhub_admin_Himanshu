...
import { generateOrderColumnHeaders } from './utils/columnHeaderUtils'
import { cn } from '@/lib/utils'
import SortIndicator from './SortIndicator'
import JsonViewerModal from './JsonViewerModal'

// Generic Order type - can be any order with at least an id
type OrderType = { id: string; [key: string]: any }

interface OrderTableProps<T extends OrderType = OrderType> {
  currentOrders: T[]
  selectedItems: string[]
  onSelectItem: (id: string) => void
  onSelectAll: () => void
  onRowClick?: (order: T, event: React.MouseEvent) => void
  columns: Array<{
    key: string
    label: string
    sortable?: boolean
    render?: (order: T, index?: number) => React.ReactNode
  }>
  loading?: boolean
  error?: string | null
  searchQuery?: string
...
export default function OrderTable<T extends OrderType = OrderType>({
  currentOrders,
  selectedItems,
  onSelectItem,
  onSelectAll,
  ...
  const [jsonOrder, setJsonOrder] = useState<T | null>(null)
  ...
  const tableHeader = (
    <thead className="hidden">
      {/* Hidden - GridColumnHeader is used instead */}
    ...
    <div className={`bg-white rounded-lg border border-gray-200 shadow-sm`} style={{ maxHeight: 'none', overflow: 'visible' }}>
      ...
      {renderHeader && (
        <GridColumnHeader
          ...
          getUniqueValues={getUniqueValues}
          sortColumn={sortState?.key || undefined}
          sortDirection={sortState?.dir || undefined}
          onSortClick={onRequestSort}
      ...
      <div className="w-full">
        ...
        >
          <table 
            ...
            style={{ 
              minWidth: '100%',
              width: `${columns.length * 150}px`  // Dynamic width based on column count
            }}
            key={`orders-table-${sortedOrders.length}`}
          >
          {renderColGroup()}
              ...
              >
                ...
                {columns.map((column) => (
                  <td key={column.key} className={cn("text-sm text-gray-900 border-r border-gray-200 whitespace-nowrap min-w-[160px]",
                    ...
                    {column.render ? column.render(order, index) : String(order[column.key as keyof T] || '')}
                  </td>
                ))}
