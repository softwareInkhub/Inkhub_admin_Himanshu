...
// Shape of all persisted UI & table state for the Content Library page.
export type ContentLibraryPageState = {
  // Table state
  pageIndex: number;
  pageSize: number;
  ...
  setSorting: (s: ContentLibraryPageState["sorting"]) => void;
  setColumnFilters: (f: ContentLibraryPageState["columnFilters"]) => void;
...
const STORAGE_KEY = "page:/content-library";

export const useContentLibraryPageStore = create<ContentLibraryPageState>()(
  persist(
    (set) => ({
      pageIndex: 0,
      pageSize: 50,
