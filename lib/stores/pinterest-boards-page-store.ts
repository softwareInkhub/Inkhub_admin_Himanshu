...
// Shape of all persisted UI & table state for the Pinterest Boards page.
export type PinterestBoardsPageState = {
  // Table state
  pageIndex: number;
  pageSize: number;
  ...
  setSorting: (s: PinterestBoardsPageState["sorting"]) => void;
  setColumnFilters: (f: PinterestBoardsPageState["columnFilters"]) => void;
...
const STORAGE_KEY = "page:/apps/pinterest/boards";

export const usePinterestBoardsPageStore = create<PinterestBoardsPageState>()(
  persist(
    (set) => ({
      pageIndex: 0,
      pageSize: 25,
