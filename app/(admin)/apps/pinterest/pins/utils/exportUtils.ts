...
export const EXPORT_FIELDS: ExportField[] = [
  { key: 'id', label: 'Pin ID', type: 'string' },
...
// Export to CSV
export const exportToCSV = (pins: Pin[], selectedFields: string[]): void => {
  ...
  link.setAttribute('download', `pinterest_pins_export_${new Date().toISOString().split('T')[0]}.csv`)
...
// Export to JSON
export const exportToJSON = (pins: Pin[], selectedFields: string[]): void => {
  // Filter fields based on selection
  const fields = EXPORT_FIELDS.filter(field => selectedFields.includes(field.key))
  ...
  const url = URL.createObjectURL(blob)
  link.setAttribute('href', url)
  link.setAttribute('download', `pinterest_pins_export_${new Date().toISOString().split('T')[0]}.json`)
  link.style.visibility = 'hidden'
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
...
export const exportToPDF = async (pins: Pin[], selectedFields: string[]): Promise<void> => {
  try {
    ...
    for (let i = 0; i < pins.length; i++) {
      ...
      if (hasImages && pin.image) {
        try {
          const imageUrl = getOptimizedImageUrl(pin.image, 150, 150)
    ...
    // Save PDF
    doc.save(`pinterest_pins_export_${new Date().toISOString().split('T')[0]}.pdf`)
  } catch (error) {
...
export const exportPins = async (
  ...
  try {
    switch (format) {
      case 'csv':
        exportToCSV(pins, selectedFields)
        break
      case 'json':
        exportToJSON(pins, selectedFields)
        break
      case 'pdf':
        await exportToPDF(pins, selectedFields)
        break
      default:
